...
class CodeInjectionApp:
    def __init__(self, root):
        ...
        # חיתוך השטח המת - גובה מינימלי להדוק מקסימלי
    ...
    def handle_leave(self, event):
        if self.chk_ghost.get(): self.root.attributes("-alpha", 0.5)
    ...
    def run_file(self):
        if self.target_file: os.startfile(self.target_file)
    ...
    def build_target(self):
        ...
        subprocess.Popen(f'start cmd /k "cd /d "{folder}" && {cmd} && pause"', shell=True)
