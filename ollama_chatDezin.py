...
class RoundedButton(tk.Canvas):
    """כפתור עם פינות מעוגלות (מצויר על קנבס)."""
    def __init__(self, parent, text="", command=None, radius=12,
                 bg="#1a1916", fg="#ffffff", hover_bg=None, border=None,
        ...
        self._shape = self.create_polygon(
            ...
            fill=bg, outline=border or bg, width=1, smooth=True)
        self._txt = self.create_text(w // 2, h // 2, text=text, fill=fg, font=font)
        self.bind("<Button-1>", lambda e: self.command() if self.command else None)
        self.bind("<Enter>", lambda e: self.itemconfig(self._shape, fill=self._hover))
        self.bind("<Leave>", lambda e: self.itemconfig(self._shape, fill=self._bg))

    def set_text(self, t):
        self.itemconfig(self._txt, text=t)

    def set_bg(self, c):
        self._bg = self._hover = c
        self.itemconfig(self._shape, fill=c, outline=c)


class RoundedFrame(tk.Canvas):
    """מסגרת עם פינות מעוגלות. הוסיפו ילדים אל ``.interior``."""
    def __init__(self, parent, radius=18, fill="#ffffff", border="#ecebe6",
                 border_width=1, pad=12, parent_bg="#f4f3f0", **kw):
    ...
    def _on_interior(self, _event):
        # grow the canvas vertically so it always wraps the content
        self.configure(height=self.interior.winfo_reqheight() + self._pad * 2)

    def _on_configure(self, event):
        w, h = event.width, event.height
        self.delete("bg")
        self.create_polygon(
            _round_rect_points(self._bw, self._bw,
                               w - self._bw, h - self._bw, self._radius),
            fill=self._fill, outline=self._cur_border, width=self._bw,
            smooth=True, tags="bg")
        self.tag_lower("bg")
    ...
    def set_border(self, color):
        self._cur_border = color
        self.itemconfig("bg", outline=color)


class RoundedIconButton(tk.Canvas):
    """כפתור אייקון עם פינות מעוגלות (אפשר עם רקע ממולא בריחוף)."""
    def __init__(self, parent, text="", command=None, size=34, radius=10,
                 fg="#a9a79d", hover_fg="#2c2a24", fill="", hover_fill="#f1f0ec",
                 font=("Segoe UI", 15), parent_bg="#ffffff"):
        ...
        self.bind("<Button-1>", lambda e: self.command() if self.command else None)
        self.bind("<Enter>", lambda e: self._draw(self._hover_fill, self._hover_fg))
        self.bind("<Leave>", lambda e: self._draw(self._fill, self._fg))
    ...
    def _draw(self, fill, fg):
        ...
        if fill:
            self.create_polygon(
                _round_rect_points(2, 2, s - 2, s - 2, r),
                fill=fill, outline=fill, smooth=True)
        self.create_text(s // 2, s // 2, text=self._text, fill=fg,
                         font=self._font)
...
class OllamaChatApp:
    ...
    BG        = "#f4f3f0"    # app background (light gray)
    CHAT_BG   = "#ffffff"   # chat transcript background
    PANEL     = "#ffffff"    # surfaces / cards
    BORDER    = "#ecebe6"    # subtle separators
    USER_FG   = "#56544c"    # user name accent (blue)
    BOT_FG    = "#5d8a6a"    # assistant name accent (green)
    TEXT_FG   = "#2c2a24"    # primary text
    MUTED_FG  = "#a9a79d"    # hints / secondary text
    USER_BUBBLE  = "#f1f0ec" # user message background
    BOT_BUBBLE   = "#ffffff" # assistant message background
    INPUT_BG  = "#ffffff"    # input field / card background
    SEND_BTN  = "#1a1916"    # send button circle color
    ...
    def _build_ui(self):
        ...
        # Rounded white card (canvas-drawn rounded corners)
        self._card = RoundedFrame(
            pad, radius=22, fill=self.INPUT_BG, border=self.BORDER,
            border_width=1, pad=12, parent_bg=self.BG,
        )
        self._card.pack(fill=tk.X)
        card = self._card.interior
        ...
        # highlight the rounded card border while typing
        self.entry.bind("<FocusIn>",
                        lambda e: self._card.set_border("#a5b4fc"), add="+")
        self.entry.bind("<FocusOut>",
                        lambda e: self._card.set_border(self.BORDER), add="+")
        ...
        # Attach (+) button — rounded icon button
        self._attachments = []   # list of {"name", "kind": "image"|"text", "data"|"text"}
        self.attach_btn = RoundedIconButton(
            toolbar, text="+", command=self._attach_file,
            size=34, radius=11, font=("Segoe UI", 18),
            fg=self.MUTED_FG, hover_fg=self.TEXT_FG,
            hover_fill="#f1f0ec", parent_bg=self.INPUT_BG,
        )
        self.attach_btn.pack(side=tk.LEFT, padx=(2, 0))

        # Attach (paperclip) button — rounded icon button
        self.clip_btn = RoundedIconButton(
            toolbar, text="📎", command=self._attach_file,
            size=34, radius=11, font=("Segoe UI", 13),
            fg=self.MUTED_FG, hover_fg=self.TEXT_FG,
            hover_fill="#f1f0ec", parent_bg=self.INPUT_BG,
        )
        self.clip_btn.pack(side=tk.LEFT, padx=(2, 0))
        ...
        # Send / Stop button — rounded dark circle with white up-arrow
        self._send_canvas = tk.Canvas(
            toolbar, width=34, height=34, bg=self.INPUT_BG,
            highlightthickness=0, bd=0, cursor="hand2",
        )
        ...
        self.send_text_btn = self._send_canvas
    ...
    def _draw_send_btn(self, active=True):
        ...
        if active:
            ...
            # stop: small white rounded square
            c.create_polygon(
                _round_rect_points(12, 12, 22, 22, 2),
                fill="white", outline="white", smooth=True, tags="stop",
            )
    ...
    def _set_busy(self, busy: bool):
        """Toggle the rounded send/stop button between send and stop."""
        self._draw_send_btn(active=not busy)
